...
in
{
  ...
  boot.initrd.systemd.network.wait-online.enable = mkForce false;
            ...
            options =
              let
                ...
                credentials = "credentials=${
                  config.sops.secrets."tygo-van-den-hurk/nas/personal/credentials".path
                }";
                premissions = "uid=${toString config.users.users.tygo.uid},gid=0";
              in
              [ "${general},${credentials},${premissions}" ];
          };
        };
    in
