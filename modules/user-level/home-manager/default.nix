...
let
  users-with-this-module-enabled = lib.attrNames (
    lib.attrsets.concatMapAttrs (
      ...
      if user-settings.init.modules.home-manager.enable then { "${user-name}" = user-settings; } else { }
    ...
    home-manager = {
      ...
      users =
        let
          ...
          home-manager-configurations = lib.attrsets.concatMapAttrs (
            __username_: __user_settings_:
            (
              if __user_settings_.init.modules.home-manager.enable then
                {
        ...
        in
        builtins.trace "(System) Loading: Home-Manager configurations: ${builtins.toJSON (builtins.attrNames home-manager-configurations)}..." home-manager-configurations;
    };
  })
else
  { }
