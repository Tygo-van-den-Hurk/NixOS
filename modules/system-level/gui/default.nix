...
in let
  enabled-GUIs = lib.attrNames (lib.attrsets.concatMapAttrs (
      GUI-name: GUI-settings:
        if (! builtins.isAttrs GUI-settings)
        then
          (
            abort (
        ...
        else if (GUI-settings.enable == true)
        then {
...
in (
  ...
  else if (amount-of-GUIs-enabled > 0)
  then
