...
let
  ...
  enabled-GUIs = lib.attrNames (
    lib.attrsets.concatMapAttrs (
      GUI-name: GUI-settings:
      if (GUI-name == "enable") then
        {
          # skip this one
        }
      else if (!builtins.isAttrs GUI-settings) then
        (abort (
      ...
      else if GUI-settings.enable then
        {
...
in
if (amount-of-GUIs-enabled > 1) then
  (
    let
      sperator = "\n\t - ";
    ...
    in
    abort ("More then 1 GUI was selected." + list-enabled-GUIs-string)
  )
else if (amount-of-GUIs-enabled > 0) then
